class QuizzQuestionsController < ApplicationController
  ...
  def question
    ...
    @word_list = @list.words_lists.where(quizz_status: false).sample
    @word_list.update(quizz_status: true)
    luck = (3..3).to_a.sample
    case luck
    when 1
      @question = 1
      good_answer = @word_list.translation
      create_quizz_question(@word_list, 1, good_answer)
    when 2
      @question = 2
      good_answer = @word_list.entry
    ...
    when 4
      @question = 4
      # @entry = @word_list.word.entry SOIT LE DEFINIR ICI SOIT DANS LA VUE
  ...
  def quizz_answer
    @list = List.find(params[:list_id])
    @word = @list.words_lists.where(quizz_status: false)
  end
